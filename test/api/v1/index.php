...
$app->get('/attendetails/:attendanceid', function($attendanceid) {
  global $db;
  $rows = $db->select("attenda","id,studentid,attendance",array('attendanceid' => $attendanceid));
  echoResponse(200, $rows);
});

$app->get('/studentsBatch/:class/:batch/:subject', function($class, $batch, $subject) {
    global $db;
    $rows = $db->select2("students","id,name,eid,phone,class,batch",array('class' => $class, 'batch' => $batch, sub5=>'%'.$subject."%" ),"LIKE '%land%'");
    echoResponse(200, $rows);
});

$app->get('/lastid/:class/:batch/:subject', function($class, $batch, $subject) {
  global $db;
  $rows = $db->select2("attendance","attendanceid",array('class' => $class, 'batch' => $batch, 'subject' => $subject), "ORDER BY attendanceid DESC LIMIT 1");
  echoResponse(200, $rows);
});
