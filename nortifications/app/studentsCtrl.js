    ...
    $scope.open = function (p,size) {
        ...
        modalInstance.result.then(function(selectedObject) {
            if(selectedObject.save == "insert"){
                ...
                p.message = selectedObject.message;
            }
        });
    };
...
app.controller('studentEditCtrl', function ($scope, $modalInstance, item, Data) {
  ...
  $scope.student = angular.copy(item);
        ...
        $scope.title = (item.id > 0) ? 'Edit Nortification' : 'Add Nortification';
        $scope.buttonText = (item.id > 0) ? 'Update Nortification' : 'Add New Nortification';
