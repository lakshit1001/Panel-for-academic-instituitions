...
$app->get('/students', function() { 
    global $db;
    $rows = $db->select("assignments","id,name,class,slideshare",array());
...
$app->post('/students', function() use ($app) { 
    ...
    global $db;
    $rows = $db->insert("assignments", $data, $mandatory);
...
$app->put('/students/:id', function($id) use ($app) { 
    ...
    $mandatory = array();
    global $db;
    $rows = $db->update("assignments", $data, $condition, $mandatory);
    if($rows["status"]=="success")
...
$app->delete('/students/:id', function($id) { 
    global $db;
    $rows = $db->delete("assignments", array('id'=>$id));
    if($rows["status"]=="success")
