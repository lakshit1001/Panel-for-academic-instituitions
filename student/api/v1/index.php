...
$app->get('/students', function() {
    global $db;
    $rows = $db->select("students","id,name,eid,feetype,amount,feediscount,feemonth,phone,father,mother,contact,sub5,class,batch",array());
    echoResponse(200, $rows);
});

$app->get('/enquiries', function() {
  global $db;
  $rows = $db->select("inquiry","name,gender,class,number,message,date,email",array());
  echoResponse(200, $rows);
});
