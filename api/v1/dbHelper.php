...
class dbHelper {
    ...
    function select($table, $columns, $where){
        try{
            ...
            $response["statement"]="select ".$columns." from ".$table." where 1=1 ". $w;
            $stmt->execute($a);
    ...
    function selectnest($table, $columns,$where ,$table2 ,$columns2, $where2 ){
        try{
            $a = array();
            $w = "";
            ...
            $stmt = $this->db->prepare("select ".$columns." from ".$table." where 1=1 and ". $where." in (select ".$columns." from ".$table." where 1=1 ". $w .")");
            $response["statement"]="select ".$columns." from ".$table." where 1=1 and ". $where." in (select ".$columns2." from ".$table2." where 1=1 ". $w .")";
            $stmt->execute($a);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
            if(count($rows)<=0){
                $response["status"] = "warning";
